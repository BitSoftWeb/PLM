...
namespace PLM_SQLDAL
{
    public class 用户操作SQL
    {
        public 用户表 UserLogin(用户表 au)
        {
            ...
            sb.Append(" INNER JOIN  SYS_BD_三级部门表 AS B ON A.部门=B.ID ");
            sb.Append(" INNER JOIN  SYS_BD_二级部门表 AS C  ON C.ID=B.Superior_ID  where (A.用户名=@username and A.密码 =@userpassword)");
